...
function createProvider(remote, config) {
  ...
  if (kind === 'github') {
    const base = assertApiBoundToRemote(kind, remote, config.githubApiBaseUrl || (remote.host === 'github.com' ? 'https://api.github.com' : `https://${remote.host}/api/v3`));
...
module.exports = { createProvider, assertApiBoundToRemote };
