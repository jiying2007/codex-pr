...
function previewHtml(webview, state, ui) {
  ...
  const dirty = state.localDirty
    ...
    : '';
  const openDisabled = state.canOpenGitHub ? '' : 'disabled';
  const openHint = state.canOpenGitHub
...
<style>
  ...
  .meta{color:var(--vscode-descriptionForeground);margin-bottom:14px}.notice{padding:10px;border-left:3px solid var(--vscode-editorWarning-foreground);background:var(--vscode-textBlockQuote-background);margin:10px 0}
...
<div class="buttons">
  <button id="copyAll">${escapeHtml(ui('复制全部', 'Copy All'))}</button>
  <button id="copyTitle" class="secondary">${escapeHtml(ui('复制标题', 'Copy Title'))}</button>
  <button id="copyBody" class="secondary">${escapeHtml(ui('复制正文', 'Copy Body'))}</button>
