...
for (const dir of ['src','test','scripts']) {
  const walk=d=>{ for(const e of fs.readdirSync(d,{withFileTypes:true})){
    if (dir==='src' && d===path.join(root,'src') && e.name==='codex-safe-core') continue;
    const p=path.join(d,e.name); if(e.isDirectory()) walk(p); else if(e.isFile()&&p.endsWith('.js')) files.push(path.relative(root,p));
...
console.log(`syntax ok: ${files.length} owned files`);
