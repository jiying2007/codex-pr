...
assert.match(docs, /Commit Receipt v4/);
assert.match(docs, /Safe Contract v2/);
assert.match(fs.readFileSync(path.join(root, 'README.md'), 'utf8'), /First successful PR draft/);
assert.match(fs.readFileSync(path.join(root, 'README.zh-CN.md'), 'utf8'), /第一次成功生成 PR/);
assert.match(docs, /ENOCHANGES/);
...
assert.doesNotMatch(`${fs.readFileSync(path.join(root,'README.md'),'utf8')}\n${fs.readFileSync(path.join(root,'README.zh-CN.md'),'utf8')}`, /Safe Core v[123]\b|Review Receipt v[123]\b|Commit Receipt v[123]\b/);

console.log('PR product documentation verified.');
