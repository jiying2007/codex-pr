...
function getText(url) {
  return new Promise((resolve, reject) => {
    const request = https.get(url, { headers: { 'User-Agent': 'codex-safe-core-sync', Accept: 'application/octet-stream' } }, response => {
...
async function verifyUpstream() {
  ...
  if (sha256(upstream.manifestBytes) !== lock.manifestSha256) fail('Locked Safe Core manifest differs from canonical upstream; run `npm run safe-core:sync` and review the diff.');
