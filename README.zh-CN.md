...
- 仓库策略唯一入口是 Safe Core 4.14.4 的 committed `.codex-safe.json` **Policy Schema v4**。
...
- Safe Core：`4.14.4` 精确 pin `25467922eeebffa93b7c820f2ffa7590c1625381`
...
Codex Change Safe 5.4.5 固定到 Core 4.14.4，并在 Product Contract 中记录 Runtime/Provider Contract v3 消费关系。Change Safe 默认模型调用仍为 0，因此不会额外暴露模型中转站凭据或 auth.json 设置；模型 Runtime 配置由 Review、Commit、Diagnose 与 Review Service 使用。Change Safe 现有 `allowInsecureHttp` 仍只服务于显式信任的 GitHub/GitLab SCM API。
